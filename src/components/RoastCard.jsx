...
export default function RoastCard({ roast, courageRating, typing = false }) {
  return (
    <div className="relative overflow-hidden rounded-xl border border-zinc-800 border-t-4 border-t-lime-400 bg-zinc-900 p-6 pt-8">
      ...
      <div className="relative flex flex-col gap-5">
        <p className="text-xl italic leading-relaxed text-zinc-100 sm:text-2xl">
          ...
          {typing && (
            <span className="ml-0.5 inline-block w-0.5 animate-pulse bg-lime-400">&nbsp;</span>
          )}
        </p>
        <div className="flex items-baseline gap-3">
          ...
          <span className="font-display text-5xl text-lime-400">
            {typing ? '—' : (courageRating ?? '–.–')}
          </span>
          {!typing && <span className="text-zinc-500">/ 10</span>}
        </div>
      </div>
    </div>
  )
