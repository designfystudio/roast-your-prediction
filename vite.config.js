...
// Dev-only convenience: serve /api/roast from `npm run dev` by adapting the
// real Vercel handler onto Vite's connect server. `vercel dev` remains the
// canonical way to test the serverless route.
function apiRoutes() {
  return {
    name: 'dev-api-routes',
    configureServer(server) {
      server.middlewares.use('/api/roast', async (req, res) => {
        ...
        const { default: handler } = await server.ssrLoadModule('/api/roast.js')
